...
/**
 * Get priority number from source name or type
 */
export function getSourcePriority(source) {
  ...
  if (sourceUpper.includes('RECREATION.GOV') || sourceUpper.includes('RECREATION_GOV')) {
  ...
  // File upload source types
  if (sourceUpper === 'PUBLIC FEDERAL' || sourceUpper.includes('PUBLIC_FEDERAL')) {
    return 85; // High priority for federal data
  }
  if (sourceUpper === 'PUBLIC STATE' || sourceUpper.includes('PUBLIC_STATE')) {
    return 80; // Manual curation level
  }
  if (sourceUpper === 'STATE AGENCY' || sourceUpper.includes('STATE_AGENCY')) {
    return 80; // Manual curation level
  }
  if (sourceUpper === 'COUNTY AGENCY' || sourceUpper.includes('COUNTY_AGENCY')) {
    return 75; // Email response level
  }
  if (sourceUpper === 'CITY AGENCY' || sourceUpper.includes('CITY_AGENCY')) {
    return 75; // Email response level
  }
...
export function shouldUpdatePark(existingPark, newParkData) {
  ...
  // RULE 3: Same priority? Update if quality improved OR if metadata needs fixing
  if (newPriority === existingPriority) {
    // Check if metadata fields need updating (data_source mismatch)
    const existingDataSource = existingPark.data_source || '';
    const newDataSource = newParkData.data_source || '';
    const needsMetadataUpdate = newDataSource && 
                                existingDataSource !== newDataSource &&
                                newPriority >= 90; // Only for high-priority sources
    ...
    if (newScore > existingScore) {
      ...
      return {
        shouldUpdate: true,
        reason: 'Fixing metadata fields (data_source)',
      };
    }
  }
  
  // RULE 4: Skip - no improvement (don't log to reduce volume)
  return {
    shouldUpdate: false,
