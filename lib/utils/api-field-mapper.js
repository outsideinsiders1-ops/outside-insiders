...
export function mapNPSToParkSchema(npsPark) {
  ...
  if (!state && npsPark.addresses && Array.isArray(npsPark.addresses) && npsPark.addresses.length > 0) {
    const address = npsPark.addresses.find(a => a.stateCode) || npsPark.addresses[0]
    if (address && address.stateCode) {
      state = address.stateCode.toUpperCase()
    }
  }
  ...
  return {
    ...
    state: state, // May still be null if no state info available
    agency: 'NPS',
