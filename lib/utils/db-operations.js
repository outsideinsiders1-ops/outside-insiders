...
export function mergeParkData(existingPark, newParkData, priority, qualityScore) {
  ...
  // Skip geometry if it's invalid (will cause PostGIS parse errors)
  if (newParkData.geometry !== undefined && newParkData.geometry !== null) {
    // If existing has no geometry, use new one
    if (!existingPark.geometry || existingPark.geometry === '') {
      // Only set if geometry is valid (has type and coordinates)
      if (newParkData.geometry.type && newParkData.geometry.coordinates) {
