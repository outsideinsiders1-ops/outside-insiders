...
export async function insertOrUpdatePark(parkData, sourceType) {
  ...
  // Check for existing park (use normalized state)
  const existingPark = await findExistingPark(parkData.name, normalizedState)
  
  if (existingPark) {
    console.log(`🔍 Found existing park: "${existingPark.name}" (${existingPark.state}) - matching "${parkData.name}" (${normalizedState})`)
